...
- **First real signed DFT result**: water + PBE/def2-tzvp on `duck@olympus.local` through PySCF 2.13.0 produced signed `chem.dft.result` artifact `art_be0fbeb3bc1abbe1` (E=-76.376421 Ha, HOMO/LUMO gap 6.964 eV, wall 0.18s, converged). The full lineage MolADT → DftRequest → DftResult is saved at `demo/dft/` and reproducible with `chimiaclaw-cli moladt-dft-demo --library water --functional pbe --out-dir demo/dft/` followed by `live dft-execute`.
- DFT crate `chimiaclaw-dft-skala` + uv worker `chimiaclaw-dft` ship the chem.dft.result schema, signing path, and PySCF backend. The Rust signer refuses unconverged or schema-mismatched results so the artifact graph never contains a fake SCF.
